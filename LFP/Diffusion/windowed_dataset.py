...
class BatteryCAFDataset(Dataset):
    ...
    def _compute_cond_stats(self):
        all_cond_vecs = [sample["cond_vec_raw"] for sample in self.flat_samples]
    ...
    def _single_window_position(self, n_valid_windows, mode="middle"):
        if n_valid_windows <= 0:
            return None
        mode = str(mode).lower()
        low_pos = min(2, n_valid_windows - 1)
    ...
    def _load_and_flatten_data(self, file_path):
        ...
        if "53C_54" in fname:
            file_source = "53C_54"
        elif "56C_19" in fname:
            file_source = "56C_19"
        elif "56C_36" in fname:
            file_source = "56C_36"
        else:
            file_source = "unknown"
...
if __name__ == "__main__":
    ...
    for batch in train_loader:
        ...
        print(f"Cond vec raw shape: {batch['cond_vec_raw'].shape}")
        if len(batch["info"]) > 0:
            print(f"First info:         {batch['info'][0]}")
            print(f"First seg idx:      {batch['segment_idx'][0].item()}")
            print(f"First window_role:  {batch['window_role'][0]}")
            print(f"First file_source:  {batch['file_source'][0]}")
        sample = {
            "input": batch["input"][0],
            "target": batch["target"][0],
            "segment_idx": batch["segment_idx"][0],
            "window_role": batch["window_role"][0],
        }
