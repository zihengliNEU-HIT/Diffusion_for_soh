# LFP Conditional CAF Diffusion and SOH Input Preparation

This repository contains the LFP version of the conditional CAF/GAF diffusion workflow. It trains and evaluates a diffusion model, generates selected-window GAF samples, and converts those generated MAT files into SOH-estimation inputs.

The LFP-specific settings in this codebase are:

```text
chemistry labels      : 53C_54 / 56C_19 / 56C_36
condition vector dim  : 24
default timesteps     : 300
...
|-- data/                              # Train/validation sliding MAT files
...
Put train/validation sliding MAT files here. This folder is used for diffusion training/testing, selected-window train/validation generation, and SOH input construction.

Expected file names follow this pattern:

```text
53C_54LFP_battery1_train_sliding.mat
53C_54LFP_battery2_val_sliding.mat
56C_19LFP_battery1_train_sliding.mat
56C_36LFP_battery1_val_sliding.mat
```
...
```text
53C_54LFP_battery8_test_sliding.mat
56C_19LFP_battery6_test_sliding.mat
56C_36LFP_battery1_test_sliding.mat
```
...
Put LFP capacity/SOH source files here:
...
`windowed_dataset.py` first removes empty windows and missing condition vectors, then selects windows using the LFP selected-window rules:
...
For a cycle with `n_valid_windows` valid windows:

```text
low    = min(2, n_valid_windows - 1)
high   = max(n_valid_windows - 3, 0)
middle = (low + high) // 2
```

If a cycle has too few valid windows, these positions fall back to an available window.
...
Important LFP fields:

```text
cond_vector_dim: 24
timesteps      : 300
chemistry      : 53C_54 / 56C_19 / 56C_36
```
