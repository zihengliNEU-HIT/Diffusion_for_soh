...
-----------------------------------------------------------------------------
Dual-path loss:
  Path A: GAF_real (1 image) -> pred_real -> MSE(pred_real, SOH)
  Path B: GAF_gen  (K imgs)  -> K preds -> MSE(pred_gen, SOH)
  Total = lambda2 * L_real + lambda1 * L_gen
CUDA_VISIBLE_DEVICES=2 python SOH_train.py
...
def dual_path_loss(model, batch, cfg, device):
    ...
    Path B: GAF_gen (B,K,1,H,W) -> flatten to (B*K,1,H,W) -> MSE.
    ...
    total =cfg.lambda2 * loss_real + cfg.lambda1 * loss_gen
    # total = loss_gen
