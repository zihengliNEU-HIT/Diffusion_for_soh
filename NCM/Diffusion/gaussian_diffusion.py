...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        print(
            f"[GaussianDiffusion] init: schedule={schedule_type}, step={timesteps}, Betarange=[{beta_start}, {beta_end}]"
        )
        ...
        try:
            ...
            print(f"Unable to save noise schedule plot: {e}")
    ...
    def p_sample_loop(self, shape, model, condition=None, clip_denoised=True):
        ...
        print(f"Start sampling: batch size={b}, shape={shape[1:]}")
        for i in reversed(range(0, self.num_timesteps)):
            if i % 50 == 0 or i == self.num_timesteps - 1:
                print(f"Sampling progress: timestep {i}/{self.num_timesteps}")
        ...
        print(
            f"Sample generation completed, range=[{img.min().item():.4f}, {img.max().item():.4f}]"
        )
        return img
    ...
    def noise_estimation_loss(self, model, x_start, t, condition=None, noise=None):
        ...
        progress = min(self._step_counter / 1000000, 1.0)
        ...
        if self._step_counter % 2000 == 0:
            with torch.no_grad():
                ...
                print(f"Reconstruction loss: {weighted_recon_loss.item():.6f}")
                print(
                    f"Weights [noise: {noise_weight:.3f}, reconstruction: {recon_weight:.3f}]"
                )
                print(f"Total loss: {loss.item():.6f}")
                print(f"Current PSNR: {psnr.item():.2f} dB")
                if psnr.item() > 38:
                    print("Target reached: PSNR > 38 dB!")
                elif psnr.item() > 35:
                    print("Good progress: PSNR > 35 dB")
                elif psnr.item() > 30:
                    print("Improving: PSNR > 30 dB")
        return loss
