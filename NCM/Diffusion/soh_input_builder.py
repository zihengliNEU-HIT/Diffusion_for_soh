...
REAL_RE = re.compile(
    "(?P<chem>1C|2C|3C)battery(?P<batt>\\d+).*?_(?P<split>train|val|test)_sliding\\.mat$",
    re.IGNORECASE,
)
GEN_RE = re.compile(
    "(?P<chem>1C|2C|3C)battery(?P<batt>\\d+).*?_(?P<split>train|val|test)_20gaf\\.mat$",
    re.IGNORECASE,
)
