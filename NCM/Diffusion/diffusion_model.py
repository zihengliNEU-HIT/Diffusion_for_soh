...
class DiffusionModel(nn.Module):
    ...
    def update_ema(self):
        ...
        if self.training_step % 1000 == 0:
            print(
                f"EMA update: step={self.training_step}, current EMA coefficient={current_ema:.6f}"
            )
    ...
    def generate_samples(
        ...
        if use_ddim:
            print(
                f"Start DDIM sampling: batch size={batch_size}, shape={shape}, step={ddim_steps}, eta={ddim_eta}"
            )
            ...
            print(f"Start DDPM sampling: batch size={batch_size}, shape={shape}")
        ...
        print(
            f"Sample generation completed: shape={samples.shape}, range=[{samples.min().item():.4f}, {samples.max().item():.4f}]"
        )
        return samples
