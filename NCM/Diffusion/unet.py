    ...
    def __init__(
        ...
        if self.has_cond:
            self.cond_mlp1 = nn.Sequential(
                nn.SiLU(), nn.Linear(cond_vector_dim, out_channels * 2)
            )
    ...
    def _apply_film(self, h, cond_vec, mlp):
        ...
        if getattr(self, "debug_film", False):
            ...
            if self._film_dbg_step % getattr(self, "_film_dbg_every", 1000) == 0:
                with torch.no_grad():
                    print(
                        f"[DownBlock FiLM] calls={self._film_dbg_step} gamma miu={gamma.mean():+.4f}, xikema={gamma.std():.4f} | beta miu={beta.mean():+.4f}, xikema={beta.std():.4f}"
                    )
        scale = 1.2
    ...
    def _apply_film(self, h, cond_vec, mlp):
        ...
        if getattr(self, "debug_film", False):
            ...
            if self._film_dbg_step % getattr(self, "_film_dbg_every", 1000) == 0:
                with torch.no_grad():
                    print(
                        f"[UpBlock FiLM] calls={self._film_dbg_step} gamma miu={gamma.mean():+.4f}, xikema={gamma.std():.4f} | beta miu={beta.mean():+.4f}, xikema={beta.std():.4f}"
                    )
        scale = 1.2
        return h * (1.0 + scale * gamma) + scale * beta
...
class MiddleBlock(nn.Module):
    ...
    def __init__(self, channels, time_emb_dim, cond_vector_dim=0):
        ...
        if self.has_cond:
            self.cond_mlp1 = nn.Sequential(
                nn.SiLU(), nn.Linear(cond_vector_dim, channels * 2)
            )
            self.cond_mlp2 = nn.Sequential(
                nn.SiLU(), nn.Linear(cond_vector_dim, channels * 2)
            )
    ...
    def _apply_film(self, h, cond_vec, mlp):
        ...
        if getattr(self, "debug_film", False):
            ...
            if self._film_dbg_step % getattr(self, "_film_dbg_every", 1000) == 0:
                with torch.no_grad():
                    print(
                        f"[MiddleBlock FiLM] calls={self._film_dbg_step} gamma miu={gamma.mean():+.4f}, xikema={gamma.std():.4f} | beta miu={beta.mean():+.4f}, xikema={beta.std():.4f}"
                    )
        scale = 1.2
        return h * (1.0 + scale * gamma) + scale * beta
...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        self.condition_embedding = ConditionEmbedding(
            condition_channels, condition_emb_dim
        )
            ...
            for _ in range(num_res_blocks):
                self.down_blocks.append(
                    DownBlock(
                        ...
                        time_emb_dim,
                        use_attention[i],
                        cond_vector_dim=self.cond_vector_dim,
                    )
                )
                in_ch = out_ch
        ...
        self.middle_block = MiddleBlock(
            in_ch, time_emb_dim, cond_vector_dim=self.cond_vector_dim
        )
    ...
    def forward(self, x, timesteps, condition=None, cond_vec=None):
        ...
        for i, down_block in enumerate(self.down_blocks):
            ...
            skips.append(h)
            if (i + 1) % self.num_res_blocks == 0 and i // self.num_res_blocks < len(
                self.downsamplers
            ):
