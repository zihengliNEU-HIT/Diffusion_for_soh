...
def test_and_save_gaf(
    ...
    print("\n=== Window-level segment PSNR and condition-vector analysis ===")
    if segment_analysis:
...
def parse_args():
    ...
    parser.add_argument("--output_dir", type=str, default="./low_4")
...
def main(args=None):
    ...
    if checkpoint_path is None:
        best_model_path = os.path.join(args.checkpoint_dir, "model_epoch_140.pt")
        if os.path.exists(best_model_path):
            checkpoint_path = best_model_path
        else:
            raise ValueError(
                "No checkpoint_path was provided and the default checkpoint was not found."
            )
