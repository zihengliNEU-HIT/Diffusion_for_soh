...
Inference:
  ...
  independently and stores an aggregate prediction and the standard deviation.
...
def run_inference(model, test_loader, device, soh_min, soh_max,
                  save_attn=True, attn_dir=None, attn_vis_n=2):
    """
    ...
    for batch in test_loader:
        ...
        for i in range(B):
            ...
            records.append({
                ...
                # 'soh_pred_mean_raw': np.median(preds_raw),
                'soh_pred_mean_raw': np.percentile(preds_raw, 90),
                # 'soh_pred_mean_raw': np.max(preds_raw),
...
def main(cfg: SOHConfig = None):
    ...
    print(f"SOH 反归一化范围: [{soh_min:.4f}, {soh_max:.4f}] Ah")
    ...
    # Inference
    print(f"开始推理，共 {len(test_ds)} 个样本 …")
    ...
    print(f"\n所有结果已保存到 {cfg.output_dir}")
    print(f"注意力图已保存到 {attn_dir}")
