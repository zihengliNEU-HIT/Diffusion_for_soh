...
def find_test_files(data_dir):
    ...
    test_paths = sorted(glob.glob(os.path.join(data_dir, '*_test_soh.mat')))
    if not test_paths:
        for name in ('test_random', 'test_low', 'test_middle', 'test_high'):
            path = os.path.join(data_dir, name)
            if os.path.isdir(path):
                test_paths.extend(sorted(glob.glob(os.path.join(path, '*_test_soh.mat'))))
...
def plot_battery_trajectory(label, gt, pred, cycle_idx, output_dir):
    ...
    ax.set_title(f'{label}  RMSE={rmse:.4f} Ah  MAE={mae:.4f} Ah  R2={r2:.4f}',
                 fontsize=9)
...
def plot_overall_grid(all_labels, all_gt, all_pred, all_cyc, cfg, output_dir):
    ...
    fig.suptitle(
        ...
        f'MAE={mae_ov:.4f} Ah  R2={r2_ov:.4f}',
        fontsize=11
    )
