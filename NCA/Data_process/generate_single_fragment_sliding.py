#!/usr/bin/env python3
"""Generate fixed-fragment MAT files for the single-fragment datasets."""
...
U_MIN = 3.0
U_MAX = 4.2
DEFAULT_MODES = ("low", "high", "random", "middle")


@dataclass(frozen=True)
class FragmentConfig:
    # Static settings copied from each original code2_22_nocycle.m script.
    dataset_name: str
    fragment_dir: str
    prefix: str
    nca: int
...
def load_batteries(data_dir: Path) -> tuple[list, list]:
    # Load the full-resolution and fixed-window MATLAB battery structs.
    full_data = loadmat(data_dir / "matlab.mat", squeeze_me=False, struct_as_record=False)
    fix_data = loadmat(data_dir / "matlab_128.mat", squeeze_me=False, struct_as_record=False)
...
def interp_to_size(values: np.ndarray, size: int = IMG_SIZE) -> tuple[np.ndarray, bool]:
    ...
    if values.size == size:
        return values, False
    return np.interp(np.linspace(1, values.size, size), np.arange(1, values.size + 1, dtype=np.float64), values), True
...
def segment_ranges(segment_mode: str) -> np.ndarray:
    # Select the fixed voltage fragment used by the requested MATLAB mode.
    mode = segment_mode.lower()
    if mode == "low":
        return np.asarray([[3.45, 3.55]], dtype=np.float64)
    if mode == "middle":
        return np.asarray([[3.70, 3.80]], dtype=np.float64)
    if mode == "high":
        return np.asarray([[4.02, 4.12]], dtype=np.float64)
    if mode == "random":
        v_start = 3.45 + np.random.rand() * (4.15 - 3.45 - 0.1)
        return np.asarray([[v_start, v_start + 0.1]], dtype=np.float64)
    raise ValueError("segment_mode must be low, middle, high, or random")
...
def process_battery(config: FragmentConfig, battery_id: int, battery_full: list, battery_fix: list,
                    segment_mode: str) -> dict:
    # Generate all MATLAB output variables for one test battery and one mode.
    ...
    for cycle_index in range(1, n_cycles + 1):
        ...
        info_values.append(np.asarray([[battery_id, cycle_index]], dtype=np.uint8))
        ranges = segment_ranges(segment_mode)
        ...
        for lo, hi in ranges:
            ...
            idx_items.append(np.asarray([[len(frag_items)]], dtype=np.uint8))
...
def main() -> None:
    # Write generated MAT files under data_for_generated/<mode>/.
    parser = argparse.ArgumentParser(description="Generate single-fragment sliding MAT files.")
    parser.add_argument("--segment-mode", choices=DEFAULT_MODES)
    parser.add_argument("--all-modes", action="store_true", help="Generate low, high, random, and middle outputs.")
    args = parser.parse_args()
    modes = DEFAULT_MODES if args.all_modes or args.segment_mode is None else (args.segment_mode,)
    for mode in modes:
        output_dir = ROOT / "data_for_generated" / mode
        output_dir.mkdir(parents=True, exist_ok=True)
        for config in DATASETS:
            data_dir = ROOT / config.dataset_name / config.fragment_dir
            battery_full, battery_fix = load_batteries(data_dir)
            nbat = min(19, len(battery_full), len(battery_fix))
            for battery_id in config.test_set:
                if battery_id > nbat:
                    print(f"Skipping battery {battery_id}: out of range.")
                    continue
                output = process_battery(config, battery_id, battery_full, battery_fix, mode)
                filename = f"{config.prefix}battery{battery_id}_02_test_sliding.mat"
                output_path = output_dir / filename
                savemat(output_path, output, do_compression=False)
                print(f"Saved -> {output_path.relative_to(ROOT)}")
    print("Done. Generated MAT files were written to data_for_generated/<mode>/.")
