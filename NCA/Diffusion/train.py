...
def plot_losses_with_psnr(
    ...
    with open(data_save_path, "w", encoding="utf-8") as f:
        ...
        max_len = max(
            ...
            len(val_losses),
            len(psnr_history),
            len(learning_rates) if learning_rates else 0,
        )
...
def main(args=None):
    ...
    np.save(
        os.path.join(args.checkpoint_dir, "cond_mean.npy"),
        train_loader.dataset.cond_mean,
    )
    np.save(
        os.path.join(args.checkpoint_dir, "cond_std.npy"), train_loader.dataset.cond_std
    )
