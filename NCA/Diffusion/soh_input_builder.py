...
MATLAB_SOURCES = {
    "025C": "./matlab/matlab025C.mat",
    "05C": "./matlab/matlab05C.mat",
...
REAL_RE = re.compile(
    "(?P<chem>025C|05C|1C)battery(?P<batt>\\d+).*?_(?P<split>train|val|test)_sliding\\.mat$",
    re.IGNORECASE,
)
GEN_RE = re.compile(
    "(?P<chem>025C|05C|1C)battery(?P<batt>\\d+).*?_(?P<split>train|val|test)_20gaf\\.mat$",
    re.IGNORECASE,
)
...
def key_to_stem(key: Tuple[str, int, str]) -> str:
    chem, batt, split = key
    return f"{chem}battery{batt}_{split}"
